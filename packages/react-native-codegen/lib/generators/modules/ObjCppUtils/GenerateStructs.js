...
const _require3 = require('./GenerateStructsForConstants'),
  generateStructsForConstants = _require3.generateStructsForConstants;
...
function getNamespacedStructName(structName) {
  return `JS::Native::_MODULE_NAME_::::${structName}`;
}

function getElementTypeForArray(property, name, moduleName, aliases) {
  const typeAnnotation = property.typeAnnotation; // TODO(T67898313): Workaround for NativeLinking's use of union type. This check may be removed once typeAnnotation is non-optional.
...
function getInlineMethodSignature(property, name, moduleName, aliases) {
...
function getInlineMethodImplementation(property, name, moduleName, aliases) {
  const typeAnnotation = property.typeAnnotation;
  ...
  function bridgeArrayElementValueIfNecessary(element) {
...
function translateObjectsForStructs(annotations, moduleName, aliases) {
